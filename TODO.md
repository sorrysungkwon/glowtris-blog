...
- [ ] **1. Clean up Hreflang (Game):** Remove the invalid `hreflang="ko"` tag from `index.html` in the game codebase since it causes duplicate content penalties.
- [ ] **2. Dwell Time Exploit (Game Embed):**
...
- [ ] **3. Advanced Schema (Blog):** Inject `FAQPage` JSON-LD schema and a 50-word TL;DR (Featured Snippet bait) block at the top of tutorial/devlog posts.
- [ ] **4. Programmatic SEO (Game):** Create 4 separate landing pages (e.g., `/sprint.html`, `/play/unblocked`) pointing to the same game engine but optimized for high-volume, low-competition keywords.
